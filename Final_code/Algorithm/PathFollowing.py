...
class PathFollowing:
    ...
    #
    #     #self.t += 1
    #     # if self.index != self.max_points-1:
    #     #     temp = self.reference_points[self.index, :]
    #     #     diff = math.sqrt((swarm_center[0]-temp[0])**2+(swarm_center[1]-temp[1])**2)
    #     #     if diff <= 1:
    #     #         self.index += 1
    #     #self.index = 0
    #     return self.reference_points[index][int(self.t[index]), :]

    def generate_trajectory(self, traj_type, total_time, dt=0.1, **kwargs):
        t = np.arange(0, total_time, dt)
        initial_point = [315, 210]
        if traj_type == 'line':
        #从起点start到终点end, 随机某一个区间
            ...
            # 参数设置（可通过kwargs覆盖）
            scale = kwargs.get('scale', 2.0)  # 控制8字大小
            speed = kwargs.get('speed', 2*np.pi / total_time)  # 控制绘制速度
            # 参数方程生成8字轨迹
            x = scale * np.sin(speed * t)
            y = scale * np.sin(speed * t) * np.cos(speed * t)  # 关键：y与x的耦合产生8字
            traj = np.stack([x, y], axis=1)
        elif traj_type == 'zigzag':
            # 参数设置
            amplitude = kwargs.get('amplitude', 1.0)  # 锯齿高度
            periods = kwargs.get('periods', 5)  # 完整周期数
            direction = kwargs.get('direction', 'x')  # 锯齿方向(x或y轴)
            # 计算锯齿波
            sawtooth = amplitude * signal.sawtooth(2 * np.pi * periods * t / total_time)
            if direction == 'x':
                x = t / total_time * 5  # 沿x轴前进
                y = sawtooth
            else:
                x = sawtooth
                y = t / total_time * 5  # 沿y轴前进
            traj = np.stack([x, y], axis=1)
        else:

            raise ValueError(f"未知的轨迹类型: {traj_type}")
        return traj
