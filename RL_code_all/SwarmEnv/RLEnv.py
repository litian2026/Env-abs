...
class CustomEnv(Wrapper):
    def __init__(self, worker_id=0):
        ...
        self.cur_dis = 0.0 
        self.pre_dis = 0.0
        self.flow_x_real = 0.0
        self.flow_y_real = 0.0
        self.flow_x_real1 = 0.0
        self.flow_y_real1 = 0.0
    ...
    def step(self, action):
        ...
        #     reward_dis = 0
        # else:
        v_ref = 16 * math.pi / 180
        speed_ratio = v_ref / self.v_max
        if step>=1:
            self.pre_dis = math.sqrt(self.previous_obs[1]*self.previous_obs[1]+self.previous_obs[0]*self.previous_obs[0])
            reward_progress = -0.1 * speed_ratio * (self.cur_dis -self.pre_dis )
        reward_dis = -0.005 * (self.cur_dis)
        reward_progress = 0.1 * (self.pre_dis - self.cur_dis)
        ...
        # move_dis = math.sqrt(v1[0]*v1[0] + v1[1]*v1[1])

        # # reward_heading = -1.0 * avr_theta_v12 / 3 * min(move_dis, 2)
        # # reward_heading = max(reward_heading, -5)
        # move_vec = np.array([x_move, y_move])
        # goal_vec = np.array([obs_new[0], obs_new[1]])

        # move_norm = np.linalg.norm(move_vec)
        # goal_norm = np.linalg.norm(goal_vec)

        # if move_norm > 1e-6 and goal_norm > 1e-6:
        #     cos_theta = np.dot(move_vec, goal_vec) / (move_norm * goal_norm)
        #     cos_theta = np.clip(cos_theta, -1.0, 1.0)
        # else:
        #     cos_theta = -1.0

        # move_scale = min(move_norm / 4.0, 1.0)
        # reward_heading = 0.5 * cos_theta * move_scale

        # near_scale = min(self.cur_dis / 8, 1.0)
        # reward_heading *= near_scale

        #reward_heading = 0 
        reward_path_following = reward_progress+reward_heading
        ...
        info = {
            ...
            "dones": done,
            "dis": self.cur_dis,
            "reward_flocking": reward_step,
            "reward_dis": reward_progress,
            "reward_heading": reward_heading,
            "reward_final": reward_arrive,
            "flow":self.flow_x,
            "flow_x": flow_x_rec,
            "flow_y": flow_y_rec
        }
    ...
    def reset(self,seed=None, options=None):
        ...
        center = np.append(center, 0)
        obs_new = np.array([
            self.current_goal[0]-center[0], self.current_goal[1]-center[1],
            0,
            math.sin(0), math.cos(0),
            self.flow_x_real, self.flow_y_real,
            self.v_max
        ])
        info = {}
        self.previous_obs = obs_new
