...
def make_output_dir(mode):
    output_dir = os.path.join(
        ROOT_DIR,
        "model_data",
        f"pretrain_static_policy_{mode}_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S"),
    )
    os.makedirs(output_dir, exist_ok=True)
    return output_dir


def transform_states(states, mode):
    if mode == "ours":
        return states / FULL_STATE_SCALE
    if mode == "no_env":
        return states[:, NO_ENV_IDX] / FULL_STATE_SCALE[NO_ENV_IDX]
    raise ValueError("Static pretraining currently supports only ours and no_env modes.")
...
def train(args):
    ...
    output_dir = args.output_dir or make_output_dir(args.mode)

    data = np.load(dataset_path, allow_pickle=True)
    raw_states = data["states"].astype(np.float32)
    states = torch.tensor(transform_states(raw_states, args.mode), dtype=torch.float32)
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, drop_last=False, generator=generator)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, drop_last=False)

    device = torch.device("cuda" if torch.cuda.is_available() and not args.cpu else "cpu")
    policy = PolicyNetwork(state_dim=states.shape[1], action_dim=2, action_low=action_low, action_high=action_high).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_states, batch_actions in train_loader:
            ...
            loss = scaled_action_mse(mu, batch_actions, action_low_tensor.to(device), action_high_tensor.to(device))
        ...
        if epoch == 1 or epoch % args.log_interval == 0 or epoch == args.epochs:
            print(
                f"Mode {args.mode} | Epoch {epoch:04d}/{args.epochs} "
...
def parse_args():
    ...
    parser.add_argument("--mode", choices=["ours", "no_env"], default="no_env")
